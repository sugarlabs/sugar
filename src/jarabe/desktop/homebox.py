...
class _CreateAIActivityPanel(Gtk.EventBox):
    ...
    def _show_generation_activity_preview(self, message=None):
        ...
        step_defs = [
            ...
            (_('Check'), _('Validating code safety and Sugar patterns'),
             'dialog-apply'),
    ...
    def _get_live_generation_review_file_data(self, file_key):
        ...
        activity_summary = _(
            'Live generation status. This will be replaced by the '
            'model-generated source as soon as it passes validation.')
        ...
        activity_code = (
            ...
            '# generated source after these checks pass:\n'
            '# - Sugar Activity subclass\n'
            '# - toolbar and canvas lifecycle\n'
            '# - Journal persistence\n'
            '# - prompt-specific learner interaction\n'
            '\n'
            'from sugar3.activity import activity\n'
            '\n'
            '\n'
        ...
        if draft_source:
            if stage == 'failed':
                activity_summary = _(
                    'The model wrote this activity.py but it did not pass '
                    'Sugar validation. The reasons are shown in the preview '
                    'panel and the chat. You can read the draft here, then '
                    'try a smaller prompt or a different model.')
                activity_meta = _(
                    'Draft source    %(provider)s - validation failed'
                ) % {
                    'provider': provider,
                }
            else:
                activity_summary = _(
                    ...
                    'checking it before previewing or installing.')
                activity_meta = _(
                    'Draft source    %(provider)s - validating - %(progress)d%%'
                ) % {
                    'provider': provider,
                    'progress': progress,
                }
        ...
        files = {
            ...
            'plan_json': {
                'title': _('aod_plan.json'),
                'summary': _('Live generation plan and validation status.'),
                'meta': common_meta,
                'language': 'json',
                'code': json.dumps(plan_data, indent=2,
                                   sort_keys=True) + '\n',
            ...
            'license': {
                ...
                'meta': common_meta,
                'language': 'text',
                'code': _('License file will be written after validation.\n'),
            },
            'activity_dir': {
                'title': _('activity/'),
                'summary': _('Sugar bundle metadata will be assembled after '
                             'the generated code passes validation.'),
                'meta': common_meta,
                'language': 'tree',
                'code': _('activity/\n  activity.info\n  activity.svg\n'),
            },
            'setup_py': {
                ...
                'code': _('from sugar3.activity import bundlebuilder\n\n'
                          '# written after activity.py validates\n'),
            },
        }
        return files[file_key]
    ...
    def _get_generated_review_file_data(self, file_key):
        ...
        metadata = {
            ...
            'plan_json': (
                _('aod_plan.json'),
                _('Records the normalized plan, provider, and validation.'),
                'json',
            ),
            'setup_py': (
                _('setup.py'),
...
'''
    ...
    def _update_preview_license_summary(self):
        ...
        self._preview_empty_note.set_text(
            _('Generation uses %s, RAG examples, the configured model, and '
    ...
    def _render_generated_activity_preview(self, result):
        ...
        for label in [_('Journal ready'), _('Project validated'),
    ...
    def _update_provider_call_status(self, stage, fraction, message):
        ...
        if stage == 'grounding':
            ...
            self._provider_status_label.set_text(
                _('Model response received. Sugar is validating the activity '
                  '- %(percent)d%%') % {'percent': percent}
            )
        elif stage == 'packaging':
            self._provider_status_label.set_text(
                _('Validation passed. Sugar is packaging the XO - '
                  '%(percent)d%%') % {'percent': percent}
            )
    ...
    def _generation_finished_cb(self, result):
        ...
        self._append_chat_status(
            _('Generated and validated. Select a part or type a refinement.'))
        if self._chat_entry is not None:
            self._chat_entry.set_placeholder_text(
                _('Ask for a refinement...'))
        self._append_sidebar_status(provider_status)
        self._append_sidebar_message(
            _('Generated and validated. Type another prompt here to refine '
              'this activity.'))
        return False
