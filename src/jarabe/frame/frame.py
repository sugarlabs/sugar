...
class Frame(object):
    ...
    def __notification_received_cb(self, **kwargs):
        ...
        if hints.get('urgency', URGENCY_NORMAL) == URGENCY_CRITICAL:
            ...
            summary_label.set_alignment(0, 1)
            summary_label.show()
            body_label = NotificationBodyLabel(kwargs['body'], max_lines=1)
            body_label.set_alignment(0, 0)
            body_label.show()
            icon.show()

            grid = NotificationGrid(icon, summary_label, body_label, padding=0)
            if hasattr(grid.props, 'margin_end'):
                grid.props.margin_end = style.DEFAULT_SPACING
            else:
                # Deprecated: Fallback for GTK < 3.12
                grid.props.margin_right = style.DEFAULT_SPACING
