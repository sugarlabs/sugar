...
# DATA_JSON_URL = 'http://www.aslo.cf/data.json'
DATA_JSON_URL = 'http://aslo-bot-master.sugarlabs.org/data.json'

class NewAsloUpdater(object):
    """
    Checks for updates using the new ASLO's data.json file
    """
    ...
    def __data_json_download_complete_cb(self, downloader, result):
        ...
        for i, bundle in enumerate(self._bundles):
            ...
            try:
                ...
                logging.debug('KeyError with %s' % bundle.get_bundle_id())
                continue
            except InvalidVersionError:
                logging.debug('InvalidVersion with %s' % bundle.get_bundle_id())
                continue
