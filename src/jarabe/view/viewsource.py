...
class SourceDisplay(Gtk.ScrolledWindow):
    ...
    def _set_file_path(self, file_path):
        ...
        childrens = self._box.get_children()
        if childrens:
    ...
    def _show_text_viewer(self):
        ...
        source_view = GtkSource.View(buffer=_buffer)
        ...
        _buffer.set_language(detected_language)
    ...
    def _show_image_viewer(self, icon=None, image=False):
        _media_box = Gtk.EventBox()
        _media_box.modify_bg(Gtk.StateType.NORMAL, Gdk.color_parse('white'))

        c = _media_box.get_child()
        if c:
            _media_box.remove(c)
        ...
        self._box.pack_start(_media_box, True, True, 0)
        self._box.show_all()

    def _show_no_file(self):
        _nofile_label = Gtk.Label()
        _nofile_label.set_text(_("Please select a file in the left panel."))

        _nofile_box = Gtk.EventBox()
        _nofile_box.modify_bg(Gtk.StateType.NORMAL, Gdk.color_parse('white'))

        _nofile_box.add(_nofile_label)
        self._box.pack_start(_nofile_box, True, True, 0)
        self._box.show_all()
