...
class AudioPalette(Palette):
    ...
    def _add_menu_item(self, label_icon, label_text, device):
        ...
        self._model_notify_level_handler_ids[device] = \
            self._models[device].connect(
                'notify::level', self.__level_changed_cb, device)
        self._muted_handler_ids[device] = \
            self._models[device].connect(
                'notify::muted', self.__muted_changed_cb, device)
    ...
    def _set_muted(self, device, muted):
        ...
        # set the slider and the icons in the buttons
        self._models[device].handler_block(
            self._model_notify_level_handler_ids[device])
    ...
    def __adjustment_changed_cb(self, widget_, device):
        ...
        try:
            ...
            # update the mute state if needed
            self._models[device].handler_block(
                self._muted_handler_ids[device])
            try:
...
class DeviceModelAudio(GObject.GObject):
    ...
    def do_get_property(self, pspec):
        ...
        logging.error('%s do_get_property %s = %s' %
                      (self.get_type(), pspec, value))
        return value

    def do_set_property(self, pspec, value):
        logging.error('%s do_set_property %s %s' %
                      (self.get_type(), pspec, value))
        if pspec.name == 'level':
